...
export default function LinkTapWidget({ device }: { device: DeviceConfig }) {
  ...
  // The poll loop (DEMO short-circuit, LAN cmd 3, server-observed off-LAN branch, command-lock
  // clearing, software volume cutoff, auto-restart, washdown transition, usage history) lives in
  // useLinkTapPoll. It shares the widget-owned refs with useLinkTapCommands — same instances.
  useLinkTapPoll({
  ...
  // Open/close senders (worker /api/control relay + LAN fallback, optimistic command lock) live in
  // useLinkTapCommands. The shared command/state-machine refs stay owned here so the poll loop and
  // the automation logic keep operating on the SAME instances.
  const { executeStartCommandRaw, executeStartCommand, executeStopCommand } = useLinkTapCommands({
